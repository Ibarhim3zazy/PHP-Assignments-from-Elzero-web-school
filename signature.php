  <link rel="stylesheet" href="css/master.css">
<?php
  function Note()
  {
    echo "<h2 style='color: #f66;'>Note : Some results may not appear on the browser 😊 </h2>";
  }
  function MyName()
  {
    echo "<h2 id='myname'>By : Ibrahim 3zazy 😁 </h2>";
  }
 ?>
  <div class="con">
    <a href="Week_1.php" title="from 1 to 5" class="weeks">Week 1</a>
    <a href="Week_2.php" title="from 6 to 12" class="weeks">Week 2</a>
    <a href="Week_3.php" title="from 13 to 19" class="weeks">Week 3</a>
    <a href="Week_4.php" title="from 20 to 29" class="weeks">Week 4</a>
    <a href="Week_5_p1.php" title="from 30 to 36" class="weeks">Week 5 p1</a>
    <a href="Week_5_p2.php" title="from 37 to 42" class="weeks">Week 5 p2</a>
    <a href="Week_6.php" title="from 43 to 52" class="weeks">Week 6</a>
    <a href="javascript:" title="soon" class="weeks">Week 7</a>
    <a href="javascript:" title="soon" class="weeks">Week 8</a>
    <a href="javascript:" title="soon" class="weeks">Week 9</a>
    <a href="javascript:" title="soon" class="weeks">Week 10</a>
  </div>
